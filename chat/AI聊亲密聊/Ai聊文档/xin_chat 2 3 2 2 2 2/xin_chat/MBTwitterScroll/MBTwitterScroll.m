...
- (MBTwitterScroll *)initTableViewWithBackgound:(UIImage*)backgroundImage avatarImage:(UIImage *)avatarImage titleString:(NSString *)titleString subtitleString:(NSString *)subtitleString buttonTitle:(NSString *)buttonTitle {
    ...
    //CGRect bounds = [[UIScreen mainScreen] bounds];
    CGRect bounds = CGRectMake(0, 20, kScreen_size.width, kScreen_size.height-64);
...
- (void) setupView:(UIImage*)backgroundImage avatarImage:(UIImage *)avatarImage titleString:(NSString *)titleString subtitleString:(NSString *)subtitleString buttonTitle:(NSString *)buttonTitle scrollHeight:(CGFloat)height type:(MBType)type {
    ...
    // Header
    self.header = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, 182)];
    [self addSubview:self.header];
    self.headerLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.header.frame.size.height - 5, self.frame.size.width, 35)];
    ...
    self.avatarImage.frame = CGRectMake(10, 130, 69, 69);
   
//    self.avatarImage = [[UIImageView alloc] initWithFrame:CGRectMake(10, 79, 69, 69)];
    
    [self.avatarImage setBackgroundImage:avatarImage forState:UIControlStateNormal];
    ...
    [self.avatarImage addTarget:self action:@selector(tapAction:) forControlEvents:UIControlEventTouchUpOutside];
#pragma mark 添加头像点击事件
 
    UILabel * titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 225, 250, 25)];
    titleLabel.text = titleString;
    
    UILabel * subtitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 177, 265, 25)];
    ...
    if (buttonTitle.length > 0) {
        headerButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - 100, 225, 80, 35)];
        ...
        headerButton.layer.cornerRadius = headerButton.frame.size.height/2;
